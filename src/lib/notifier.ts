...
export function findCandidates( observed: readonly Dir[] ): ReverseResult {
  ...
  const obsIdx = observed.map( ( o ) => DIRS.findIndex( ( d ) => d.dx === o.dx && d.dy === o.dy && d.dz === o.dz ) )
  ...
  matches.sort(
    ( a, b ) =>
      Math.abs( a.example.x ) + Math.abs( a.example.z ) - ( Math.abs( b.example.x ) + Math.abs( b.example.z ) )
  )
